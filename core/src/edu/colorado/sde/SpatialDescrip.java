...
import edu.biogeomancer.colorado.greg.parse.query.*;
import edu.biogeomancer.colorado.greg.parse.query.impl.*;
...
class QueryHandler {
  ...
  public GazetteerQueryType getGazetteerQuery(InputSource pSource) throws JAXBException {
	  JAXBContext context = JAXBContext.newInstance("edu.biogeomancer.colorado.greg.parse.query", this.getClass().getClassLoader());
	  Unmarshaller unmarshaller = context.createUnmarshaller();
	  return (GazetteerQueryType) unmarshaller.unmarshal(pSource);
  }

  public String getGazetteerQueryAsString(GazetteerQueryType query) throws JAXBException {
  ...
  //dim1 iterates over reports returned (max 100, see gaz.properties to change).
  //dim2 - facet of the report (names, classifications, footprints, etc.). See Constants.java.
  //dim3 - 0..n-1 repetitions of the facet. For example if the facet is names and there are n 
  //  names, dim 3 will be n-1.
  //dim4 - 0..i-1 for the i categories of the facet. ex. classification facet has a term, a term id,
  ...
  //
  //no form of results ranking is being done here. adl has a hook for it in a user-supplied 
  //comparison function that the gazetteer uses, although there is a significant impact on
  //performance to doing so from within the adl server.
  
   //takes a list of reports and navigates the in-memory JAXB datastructure, puts desired information
   //into the more straightforwardly accessed datastructure Object[][][] described above.
}

public class SpatialDescrip {
  /***
  this is the spatial description component. it gets jobs
  ...
  ***/
  ...
  public void init() {
    ...
    db_urls[0] = conn_url + host + ":" +  port + "/" + gazrepu + "?" + "user=" + conn_owner + "&" + 
        "password=" + conn_pass;
     
    db_urls[1] = conn_url + host + ":" + port + "/" + gazseachu + "?" + "user=" + conn_owner + "&" + 
        "password=" + conn_pass;
    ...
    try {
      ...
      exec_i[3] = sh;
      exec_i[4] = geod;
      exec_i[Constants.max_exec_params-2] = ""; //username not needed for gazetteer
      exec_i[Constants.max_exec_params-1] = ""; //password not needed for gazetteer
    }
  }

  public static void addAppender(Logger log, String backup_path, ResourceBundle rp) {
    ...
    try {
      FileOutputStream output = 
        new FileOutputStream(fu.addTrailingSlash(log_path) + default_log, true);
  ...
  public FeatureInfo[] lookupFeature(String locality) {
    ...
    try {
      //gbh defaulting to NameQuery. Other query types implemented and ready to use
      ...
      query_string = 
        query_string.replace("xmlns=\"http://query.parse.greg.colorado.biogeomancer.edu\"", "");
      ...
      for (ii=0; ii<results.length; ii++) {
        ...
        fifo.geodeticDatum = DatumManager.getInstance().getDatum("WGS84"); //!!!!this is correct because specified that all
