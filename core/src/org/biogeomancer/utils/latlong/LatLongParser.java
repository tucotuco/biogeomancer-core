...
public abstract class LatLongParser {
	...
	private static ArrayList<LatAndLong> latAndLongParseHelper(String s) {
		...
		// 10D 36M N, 95D 39M E
		Pattern p1 = Pattern.compile("[0-9]+[Dd\u00b0]\\s*[0-9]+[Mm']\\s*[NnSsEeWw],[0-9]+[Dd\u00b0]\\s*[0-9]+[Mm']\\s*[NnSsEeWw]");
		...
		if (m1.matches()) {
			...
			String direction0 = Pattern.compile("[NnSsEeWw]").matcher(latlongstr[0]).group();
			String direction1 = Pattern.compile("[NnSsEeWw]").matcher(latlongstr[1]).group();
			if (direction0 != null && direction1 != null) {
				if (direction0.equals("w") || direction0.equals("W")
					...
					// find degrees
					info.degmag0 = Integer.parseInt(Pattern.compile("[0-9]+(?=[Dd\u00b0])").matcher(latlongstr[0]).group());
					// find minutes
					info.minmag0 = Integer.parseInt(Pattern.compile("[0-9]+(?=[Mm'])").matcher(latlongstr[0]).group());
					info.direction1 = direction1;
					info.direction1defined = true;
					info.latlong1 = "latitude";
					info.latlong1defined = true;
					// find degrees
					info.degmag1 = Integer.parseInt(Pattern.compile("[0-9]+(?=[Dd\u00b0])").matcher(latlongstr[1]).group());
					// find minutes
					info.minmag1 = Integer.parseInt(Pattern.compile("[0-9]+(?=[Mm'])").matcher(latlongstr[1]).group());
					...
					info.latlong0defined = true;
					// find degrees
					info.degmag0 = Double.parseDouble(Pattern.compile("[0-9]+(?=[Dd\u00b0])").matcher(latlongstr[0]).group());
					// find minutes
					info.minmag0 = Double.parseDouble(Pattern.compile("[0-9]+(?=[Mm'])").matcher(latlongstr[0]).group());
					info.direction1 = direction1;
					info.direction1defined = true;
					info.latlong1 = "longitude";
					info.latlong1defined = true;
					// find degrees
					info.degmag1 = Double.parseDouble(Pattern.compile("[0-9]+(?=[Dd\u00b0])").matcher(latlongstr[1]).group());
					// find minutes
					info.minmag1 = Double.parseDouble(Pattern.compile("[0-9]+(?=[Mm'])").matcher(latlongstr[1]).group());
				}
			}
			...
			} else if (info.latlong1.equals("latitude") && info.latlong0.equals("longitude")) {
	 ...
	 */
	private static ArrayList<LatAndLong> latAndLongParseHelper(String s1, String s2) {
		// Create the ArrayList<LatLong> to hold candidates
		ArrayList<LatAndLong> retval = new ArrayList<LatAndLong>();
	...
	private static ArrayList<DegMinSec> parseLatOrLong(String numstr) {
		...
		/*
		 * while (!areDone) { //ArrayList<DegMinSec> resultSet = new ArrayList<DegMinSec>();
