...
import org.gbif.datatester.DataTest;
import org.gbif.datatester.DataType;
import org.gbif.datatester.GenericRecord;
import org.gbif.datatester.GenericRecordSet;
import org.gbif.datatester.KnownConcepts;
import org.gbif.datatester.RamRecord;
import org.gbif.datatester.RecordFilter;
import org.gbif.datatester.RecordSetPartitioner;
...
/**
 * <p>
 * Manager class responsible for data validation.
 * </p>
 * 
...
public class ValidationManager implements RecordFilter {
  ...
  /**
   * <p>
   * Internal class to handle exceptions.
   * </p>
   */
  ...
  /**
   * <p>
   * Original biogeomancer record set.
   * </p>
   */
  ...
  /**
   * <p>
   * GenericRecordSet wrapping the BG record set.
   * </p>
   */
  ...
  /**
   * <p>
   * Indicates if the GenericRecordSet was loaded.
   * </p>
   */
  ...
  /**
   * <p>
   * Map to store OutlierTest results. Key is Concept and value is
   * NumericOutlierResult.
   * </p>
   */
  private final Map numericOutlierResults = new HashMap();

  /**
   * <p>
   * Another direct reference to longitude outliers just to optimize performance
  ...
  /**
   * <p>
   * Another direct reference to latitude outliers just to optimize performance
   * when displaying results.
   * </p>
   */
  ...
  /**
   * <p>
   * Wrapper object storing information about itinerary outlier results.
   * </p>
   */
  ...
  /**
   * <p>
   * Wrapper object storing information about elevation error results.
   * </p>
   */
  ...
  /**
   * <p>
   * Wrapper object storing information about geographic error results.
   * </p>
   */
  ...
  /**
   * <p>
   * List of environmental variables to be tested (used only when the
   * environmental outlier test is active).
   * </p>
   */
  ...
  /**
   * <p>
   * Concepts related to each environmental variable to be tested (used only when
   * the environmental outlier test is active).
   * </p>
   */
  private final Set<Concept> envConcepts = new HashSet<Concept>();

  /**
   * <p>
   * Wrapper object to read environmental outlier results.
   * </p>
   */
  ...
  /**
   * <p>
   * Wrapper object to read ecological outlier results.
   * </p>
   */
  ...
  /**
   * <p>
   * List of record indexes that can't be used by geographic, environmental and
   * itinerary outlier tests because they have either more than one "valid"
   * interpretation or all interpretations "invalid" according to previous tests.
   * This property is only used when there are multiple interpretations associated
   * with the same record!
   * </p>
   */
  private final List<Integer> skippedRecords = new ArrayList<Integer>();

  /**
   * <p>
   * Map (record index pointing to interpretation index) to indicate the remaining
   * interpretation that should be tested by geographic, environmental and
   * itinerary outlier tests. This property is only used when there are multiple
   * interpretations associated with the same record!
   * </p>
   */
  private final Map<Integer, Integer> remainingInterpretations = new HashMap<Integer, Integer>();

  /**
   * <p>
   * List of smaller record sets after partitioning the original one by scientific
   * name. So each partition contains records related to a particular species.
   * </p>
   */
   ...
   * 
   * @param properties
   *          Object containing configuration (key/value pairs).
   * @param bgRecSet
   *          BioGeomancer record set
   */
  ...
  /**
   * <p>
   * Prints validation results associated with a specific original record.
   * </p>
   * 
   * 
   * @param recIndex
   ...
   */
  public void displayResults(int recIndex)
      throws ValidationManager.ValidationManagerException {
    ...
    try {
      ...
      if (this.geographicErrorResult != null) {
        ...
        GeographicError geographicError = this.geographicErrorResult
            .getError(recIndex);
        ...
        if (geographicError != null) {
          ...
          if (geographicError.hasErrorInCountry()) {
            ...
            System.out.println("\t\t\tCountry feature--> "
                + geographicError.getCountryFeature());
            System.out.println("\t\t\t\tdistance measured = "
                + geographicError.getDistance(BGConcepts.COUNTRY));
          }
          if (geographicError.hasErrorInStateProvince()) {

            System.out.println("\t\t\tState/province feature--> "
                + geographicError.getStateProvinceFeature());
            System.out.println("\t\t\t\tdistance measured = "
                + geographicError.getDistance(BGConcepts.STATE_PROVINCE));
          }
          if (geographicError.hasErrorInCounty()) {

            System.out.println("\t\t\tCounty feature--> "
                + geographicError.getCountyFeature());
            System.out.println("\t\t\t\tdistance measured = "
                + geographicError.getDistance(BGConcepts.COUNTY));
          }
        }
      }
      ...
      if (this.elevationErrorResult != null) {
        ...
        ElevationError elevationError = this.elevationErrorResult
            .getError(recIndex);
        ...
        if (elevationError != null) {
          ...
          System.out.println("\t\t\tCalculated elevation--> "
              + elevationError.getCalculatedElevation() + " (min = "
              + elevationError.getCalculatedMinimumElevation() + " / max = "
              + elevationError.getCalculatedMaximumElevation() + ")");
        }
      }
      ...
      if (this.ecologicalOutlierResult != null) {
        ...
        if (this.ecologicalOutlierResult.isOutlier(recIndex)) {
          ...
          System.out
              .println("\t\tEcological outlier: yes (probability of presence = "
                  + this.ecologicalOutlierResult.getProbability(recIndex)
                  * 100
                  + "%)");
        } else {
      ...
      if (this.itineraryResult != null) {
        ...
        Collection itineraryOutliers = this.itineraryResult
            .getOutliers(recIndex);
        ...
        if (itineraryOutliers.isEmpty()) {
          ...
          for (Iterator i = itineraryOutliers.iterator(); i.hasNext();) {
            ...
            double calculatedDistanceInKilometers = outlier
            ...
            System.out
                .println("\t\t\tVery strange that someone travelled "
                    + calculatedDistanceInKilometers
                    + " kilometers in "
                    + numberOfDays
                    + " day(s) by the time of "
                    + date
                    + ". The maximum distance should not exceed "
                    + maximumDistanceInKilometers
                    + " kilometers per day. Comparison performed with record identified by "
                    + otherRecordId + ".");
          }
        }
      }
      ...
      if (this.latOutliers != null && this.latOutliers.skippedRecord(recIndex)) {
        ...
        System.out
            .println("\t\tSkipped geographical and environmental outlier tests (not enough records for the species).");
      } else {

        if (this.latOutliers != null && this.longOutliers != null) {

          System.out.println("\t\tLatitude:");
          System.out.println("\t\t\tOutlierness--> "
              + this.latOutliers.getOutlierness(recIndex));
          System.out.println("\t\t\tOutlier--> "
              + this.latOutliers.isOutlier(recIndex));
          System.out.println("\t\tLongitude:");
          System.out.println("\t\t\tOutlierness--> "
              + this.longOutliers.getOutlierness(recIndex));
          System.out.println("\t\t\tOutlier--> "
              + this.longOutliers.isOutlier(recIndex));
        }

        if (this.environmentalOutlierResult != null) {

          EnvironmentalOutlier environmentalOutlier = this.environmentalOutlierResult
              .getOutlier(recIndex);
          ...
          if (environmentalOutlier == null) {
            ...
            System.out
                .println("\t\t\tFailed in "
                    + environmentalOutlier.getOutlierVariables().size()
                    + " layers");
            System.out.println("\t\t\tAverage outlierness--> "
                + environmentalOutlier.getAverageOutlierness());
          }
        }
      }
      ...
      throw new ValidationManager.ValidationManagerException(exception
          .getMessage());
    }
  }
  ...
  /**
   * <p>
   * Prints validation results associated with a specific interpretation.
   * </p>
   * 
   * 
   * @param recIndex
   ...
   * @throws ValidationManager.ValidationManagerException
   */
  public void displayResults(int recIndex, int georefIndex)
      throws ValidationManager.ValidationManagerException {
    ...
    try {
      ...
      if (this.geographicErrorResult != null) {
        ...
        GeographicError geographicError = this.geographicErrorResult.getError(
      ...
      if (this.elevationErrorResult != null) {
        ...
        ElevationError elevationError = this.elevationErrorResult.getError(
            recIndex, georefIndex);
      ...
      if (this.itineraryResult != null) {
        ...
        Collection itineraryOutliers = this.itineraryResult.getOutliers(
            recIndex, georefIndex);
      ...
      if (this.latOutliers != null
          && this.latOutliers.skippedRecord(recIndex, georefIndex)) {
        ...
        if (this.latOutliers != null && this.longOutliers != null) {
          ...
          System.out.println("\t\tLatitude:");
          System.out.println("\t\t\tOutlierness--> "
              + this.latOutliers.getOutlierness(recIndex, georefIndex));
          System.out.println("\t\t\tOutlier--> "
              + this.latOutliers.isOutlier(recIndex, georefIndex));
          System.out.println("\t\tLongitude:");
          System.out.println("\t\t\tOutlierness--> "
              + this.longOutliers.getOutlierness(recIndex, georefIndex));
          System.out.println("\t\t\tOutlier--> "
              + this.longOutliers.isOutlier(recIndex, georefIndex));
        }

        if (this.environmentalOutlierResult != null) {

          EnvironmentalOutlier environmentalOutlier = this.environmentalOutlierResult
              .getOutlier(recIndex, georefIndex);
  ...
  public void displaySummary() {
    ...
    // NOTE: The following information doesn't make sense anymore to be displayed
    // here, since outliers
    // are now calculated per species. The methods getMinimumValue and
    // getMaximumValue are still available,
    ...
    // System.out.println("\t\tLatitude:");
    // System.out.println("\t\t\tMinimum value of the acceptable range--> " +
    // this.latOutliers.getMinimumValue());
    // System.out.println("\t\t\tMaximum value of the acceptable range--> " +
    // this.latOutliers.getMaximumValue());
    // System.out.println("\t\tLongitude:");
    // System.out.println("\t\t\tMinimum value of the acceptable range--> " +
    // this.longOutliers.getMinimumValue());
    // System.out.println("\t\t\tMaximum value of the acceptable range--> " +
    // this.longOutliers.getMaximumValue());
    // }
    //
    // if ( this.environmentalOutlierResult != null ) {
    //
    ...
    //
    // NumericOutlierResult numericOutlierResult = this.getNumericOutlierResult(
    // concept );
    //
    // System.out.println("\t\t" + concept.getId() + ":");
    // System.out.println("\t\t\tMinimum value of the acceptable range--> " +
    // numericOutlierResult.getMinimumValue());
    // System.out.println("\t\t\tMaximum value of the acceptable range--> " +
    // numericOutlierResult.getMaximumValue());
    // }
    // }
  }
  ...
  /**
   * <p>
   * Indicates if validation is enabled or not.
   * </p>
   * 
   * 
  ...
  public boolean enabled() {
    ...
    String enableValidationStr = this.properties
        .getProperty("Validation.enabled");

    if (enableValidationStr != null
        && Boolean.parseBoolean(enableValidationStr)) {
  ...
  /**
   * <p>
   * Returns the corresponding DataTester Target for a single original record.
   * </p>
   * 
   * 
   * @param recIndex
  ...
  public Target getDataTesterTarget(int recIndex) {
    ...
    Map dataMap = new OriginalRecordDataMap(
        (OriginalRecordSet) this.genericRecordSet, recIndex);

    GenericRecord genericRecord = new RamRecord(
        OriginalRecordSet.ID_DEFINITION, dataMap);
  ...
  /**
   * <p>
   * Returns the corresponding DataTester Target for a single interpretation
   * record.
   * </p>
   * 
   * 
  ...
  public Target getDataTesterTarget(int recIndex, int georefIndex) {
    ...
    Map dataMap = new InterpretationDataMap(
        (InterpretationRecordSet) this.genericRecordSet, recIndex, georefIndex);

    GenericRecord genericRecord = new RamRecord(
        InterpretationRecordSet.ID_DEFINITION, dataMap);
  ...
  /**
   * <p>
   * Returns the version of the DataTester framework.
   * </p>
   * 
   * 
  ...
  public String getDataTesterVersion() {
    ...
    ResourceBundle resourceBundle = ResourceBundle
        .getBundle("org.gbif.datatester.DataTester");
  ...
  /**
   * <p>
   * Returns the first AssignedTag occurrence of the specified type, or null if
   * there has been no assignment of the specified type.
   * </p>
   * 
   * 
   * @param resultManager
   *          Related ResultManager instance.
   * @param tagId
   *          Tag id.
   * @return First AssignedTag of the specified type or null.
   */
  public AssignedTag getFirstAssignedTag(ResultManager resultManager,
      String tagId) {
  ...
  /**
   * <p>
   * Returns the first AssignedTag occurrence of the specified type related to the
   * specified target, or null if there has been no assignment matching these
   * requirements.
   * </p>
   * 
   * 
  ...
  /**
   * <p>
   * Returns an instance of ItineraryResult.
   * </p>
   * 
   * 
  ...
  /**
   * <p>
   * Returns an instance of NumericOutlierResult related with the specified
   * Concept.
   * </p>
   * 
   * 
  ...
  /**
   * <p>
   * Indicates if a {@link GenericRecord} should be skipped for some reason. This
   * method is implemented according to the RecordFilter interface from the
   * DataTester framework and is not supposed to be called from any other context
   * except the GenericRecordSet implementation being used.
   * </p>
   * 
   * @param genericRecord
   *          {@link GenericRecord} to be verified.
  ...
  public boolean skipRecord(GenericRecord genericRecord) {
    ...
    try {
      ...
      Integer georefIndex = genericRecord
          .getIntegerValue(BGConcepts.GEOREF_INDEX);

      Rec rec = this.bgRecSet.recs.get(recIndex);

      // Do not skip interpretation if it's the only one associated with
      // the record
      ...
      // From here we only deal with multiple interpretations associated
      // with the same record

      // Skip interpretation if the corresponding record is already marked
      // to be skipped
      ...
      if (this.remainingInterpretations.containsKey(recIndex)) {
        ...
        if (nonSuspectInterpretations == 1) {
          ...
          // If there is one and only one "valid" interpretation, use
          // it and
          // store it in the cache
          this.remainingInterpretations
              .put(recIndex, lastNonSuspectGeorefIndex);
          return false;
        } else {

          // Record should be skipped when there are no "valid"
          // interpretations
  ...
  /**
   * <p>
   * Returns the current validation target: OriginalRecords or Interpretations.
   * Defaults to OriginalRecords.
   * </p>
   * 
   * 
   * @return String "OriginalRecords", "Interpretations" or empty String if it was
   *         not set.
   * 
   * @throws ValidationManager.ValidationManagerException
   */
  public String target() throws ValidationManager.ValidationManagerException {
    ...
    String validationTargetStr = this.properties
  ...
  public void validate() throws ValidationManager.ValidationManagerException {
    ...
    System.out
        .println("POST-PROCESSING\t\tStarting Validation using DataTester version "
            + this.getDataTesterVersion());

    this.loadRecordSet();

    String detectGeographicErrors = this.properties
        .getProperty("Validation.detectGeographicErrors");
    ...
    String detectElevationErrors = this.properties
        .getProperty("Validation.detectElevationErrors");
    ...
    String detectEcologicalOutliers = this.properties
        .getProperty("Validation.detectEcologicalOutliers");
    ...
    String detectGeographicOutliers = this.properties
        .getProperty("Validation.detectGeographicOutliers");
    ...
    String detectEnvironmentalOutliers = this.properties
        .getProperty("Validation.detectEnvironmentalOutliers");
    ...
    String detectItineraryOutliers = this.properties
        .getProperty("Validation.detectItineraryOutliers");
  ...
  /**
   * <p>
   * Ecological outlier detection.
   * </p>
   * 
   * 
   * @throws ValidationManager.ValidationManagerException
   */
  private void detectEcologicalOutliers()
      throws ValidationManager.ValidationManagerException {

    System.out
        .println("POST-PROCESSING\t\tStarting Ecological Outlier Detection");
    ...
    try {
      ...
      probabilityProxyTest.initialize(testInstanceId, this.knownConcepts,
          parameters);
      probabilityProxyTest.test(this.genericRecordSet);

      this.ecologicalOutlierResult = new EcologicalOutlierResult(this,
          probabilityProxyTest.getResult());
    } catch (DataTesterException exception) {
    ...
    System.out
        .println("POST-PROCESSING\t\tFinished Ecological Outlier Detection");
  }
   ...
   * @throws ValidationManager.ValidationManagerException
   */
  private void detectElevationErrors()
      throws ValidationManager.ValidationManagerException {
    ...
    try {
      ...
      parameters.put("minimumElevationConcept", BGConcepts.MINIMUM_ELEVATION
          .getId());
      parameters.put("maximumElevationConcept", BGConcepts.MAXIMUM_ELEVATION
          .getId());
      parameters.put("uncertaintyConcept", BGConcepts.COORDINATE_UNCERTAINTY
          .getId());
      parameters.put("rasterDataReader",
          "org.biogeomancer.validation.DivaLookup");
      parameters.put("rasterAttributeId", "Elevation");

      DataTest elevationErrorTest = new ElevationErrorTest();
      elevationErrorTest.initialize(testInstanceId, this.knownConcepts,
          parameters);

      elevationErrorTest.test(this.genericRecordSet);
      this.elevationErrorResult = new ElevationErrorResult(this,
          elevationErrorTest.getResult());
    } catch (DataTesterException exception) {
   ...
   * @throws ValidationManager.ValidationManagerException
   */
  private void detectEnvironmentalOutliers()
      throws ValidationManager.ValidationManagerException {

    System.out
        .println("POST-PROCESSING\t\tStarted Environmental Outlier Detection");

    String testInstanceId = "5";

    // Get environmental outlier threshold
    String environmentalOutlierThreshold = this.properties
        .getProperty("Validation.environmentalOutlierThreshold");
    ...
    try {
      ...
      for (Concept envConcept : this.envConcepts) {
        ...
        this.environmentalOutlierResult = new EnvironmentalOutlierResult(this,
            this.envConcepts, threshold);
      }
    } catch (DataTesterException exception) {
    ...
    System.out
        .println("POST-PROCESSING\t\tFinished Environmental Outlier Detection");
  }
   ...
   * @throws ValidationManager.ValidationManagerException
   */
  private void detectGeographicErrors()
      throws ValidationManager.ValidationManagerException {

    System.out
        .println("POST-PROCESSING\t\tStarting Geographic Error Detection");
    ...
    try {
      ...
      parameters.put("longitudeConcept", BGConcepts.LONGITUDE.getId());
      parameters.put("latitudeConcept", BGConcepts.LATITUDE.getId());
      parameters.put("administrativeRegionsConcepts", BGConcepts.COUNTRY
          .getId()
          + ","
          + BGConcepts.STATE_PROVINCE.getId()
          + ","
          + BGConcepts.COUNTY.getId());
      parameters.put("administrativeRegionsFeatures",
          "Country,StateProvince,County");
      parameters.put("featureDataReader",
          "org.biogeomancer.validation.DivaLookup");

      DataTest geographicErrorTest = new GeographicErrorTest();
      geographicErrorTest.initialize(testInstanceId, this.knownConcepts,
          parameters);

      geographicErrorTest.test(this.genericRecordSet);
      this.geographicErrorResult = new GeographicErrorResult(this,
          geographicErrorTest.getResult());
    } catch (DataTesterException exception) {
    ...
    System.out
        .println("POST-PROCESSING\t\tFinished Geographic Error Detection");
  }
   ...
   * @throws ValidationManager.ValidationManagerException
   */
  private void detectGeographicOutliers()
      throws ValidationManager.ValidationManagerException {

    System.out
        .println("POST-PROCESSING\t\tStarting Geographic Outlier Detection");
    ...
    System.out
        .println("POST-PROCESSING\t\tFinished Geographic Outlier Detection");
  }

  /**
   * <p>
   * Itinerary outlier detection.
   * </p>
   * 
   * 
   * @throws ValidationManager.ValidationManagerException
   */
  private void detectItineraryOutliers()
      throws ValidationManager.ValidationManagerException {

    System.out
        .println("POST-PROCESSING\t\tStarting Itinerary Outlier Detection");
    ...
    try {
      ...
      if (this.genericRecordSet.size() >= itineraryTest.minNumberOfRecords()) {
        ...
        this.itineraryResult = new ItineraryResult(this, itineraryTest
            .getResult());
      }
    } catch (DataTesterException exception) {
    ...
    System.out
        .println("POST-PROCESSING\t\tFinished Itinerary Outlier Detection");
  }
  ...
  private void detectNumericOutliers(String testInstanceId, Concept concept)
    ...
    for (GenericRecordSet partition : this.recordSetPartitions) {
      ...
      if (partition.size() >= minNumberOfRecords) {
        ...
        numericOutlierTest.initialize(testInstanceId, this.knownConcepts,
            parameters);
        numericOutlierTest.test(partition);

        numericOutlierResult.addResult(scientificName, numericOutlierTest
            .getResult());
      }
    }
  ...
  /**
   * <p>
   * Indicates if some problem was detected with the specified interpretation
   * during the individual tests (tests that can run on each record separately, as
   * opposed to tests that depend on the record set context).
   * </p>
   * 
   * @param recIndex
   *          Index of the original record in the record set.
   ...
   * @throws ValidationManager.ValidationManagerException
   */
  private boolean isSuspectAccordingToIndividualTests(int recIndex,
      int georefIndex) throws ValidationManager.ValidationManagerException {
  ...
  /**
   * <p>
   * Loads environmental data when the environmental outlier test is enabled.
   * </p>
   * 
   * 
   * @throws ValidationManager.ValidationManagerException
  ...
  private void loadEnvironmentalData()
    ...
    if (detectEnvironmentalOutliers != null
      ...
      String environmentalVariables = this.properties
          .getProperty("Validation.environmentalVariables");

      // Check additional mandatory parameter
      if (environmentalVariables == null
          || environmentalVariables.length() == 0) {
      ...
      while (stringTokenizer.hasMoreTokens()) {
        ...
        // Add the concept using its name as the identifier, and double
        // as the data type
      ...
      if (this.target().equals("OriginalRecords")) {
        ...
        for (Rec record : bgRecords) {
          ...
          for (int i = 0; i < this.envVars.size(); ++i) {
            ...
            if (longStr != null && longStr.length() > 0 && latStr != null
                && latStr.length() > 0) {
        ...
        for (Rec record : bgRecords) {
          ...
          for (Georef georef : georefs) {
            ...
            for (int i = 0; i < this.envVars.size(); ++i) {
              ...
              String salValue = sal.lookup(longitude.toString(), latitude
                  .toString(), attribute);
              if (salValue != null && salValue.length() > 0) {
  ...
  /**
   * <p>
   * Loads a DataTester GenericRecordSet based on the Biogeomancer RecSet. If the
   * environmental outlier test is active then first it loads environmental data
   * in the BG record structure.
   * </p>
   * 
   * @throws ValidationManager.ValidationManagerException
   */
    ...
    if (!this.isLoaded) {
      ...
      try {
        ...
        if (this.target().equals("OriginalRecords")) {
          ...
          // Generic record set needs to know in advance the universe
          // of concepts
          ...
          this.genericRecordSet = new InterpretationRecordSet(bgRecSet,
              concepts);
        }
  ...
  /**
   * <p>
   * Populates the recordSetPartitions property with Generic Record Sets
   * containing records for each distinct value of the concept that was passed as
   * a parameter.
   * </p>
   * 
   * 
   ...
   * @throws DataTesterException
   */
  private void partitionRecordSet(Concept concept) throws DataTesterException {
    ...
    this.recordSetPartitions = RecordSetPartitioner.getPartitions(
        this.genericRecordSet, criteria);
  }
}
