...
public class GeorefManager extends BGManager {
  ...
  public static void main(String[] args) throws Exception {
    ...
    case REMOVE_USERFEATURES:
      if (arg1 == null)
        System.out
            .println("REMOVE_USERFEATURE requires feature_id as an input parameter (-1 to delete all user features from the BG Gazetteer.");
      else {
    ...
    System.out.println("\nElapsed test time: " + (endtime - starttime)
        + " (ms)\n");
    gm.shutdown();
  }
  ...
  public boolean georeference(Rec rec, GeorefPreferences prefs) {
    ...
    try {
      if (prefs.locinterp == null || prefs.locinterp.equalsIgnoreCase("yale")) {
        ...
        this.yaleLocInterp.doParsing(rec, "verbatimlatitude", gdm,
            prefs.language);
        this.yaleLocInterp.doParsing(rec, "verbatimlongitude", gdm,
            prefs.language);
        this.yaleLocInterp.doParsing(rec, "verbatimcoordinates", gdm,
            prefs.language);
      ...
      System.out
          .println("Error in GeorefManager.georeference(Rec, GeorefPreferences)");
    }
    ...
    spatialDescriptionManager.doNewSpatialDescription(rec, prefs.showSource,
        prefs.showType);
  ...
  public boolean georeferenceAllUIUCFirst()
    ...
    for (int i = 0; i < this.recset.recs.size(); i++) {
      ...
      spatialDescriptionManager.doNewSpatialDescription(r, true, true);
  ...
  /*
   * public boolean georeference(Rec rec, int featureid) { // In this method we
   * are going to add a single georeference // to the rec based solely on the
   * feature referenced by featureid // with locType F. This assumes the the
   * rest of the Rec has already been // georeferenced.
   * spatialDescriptionManager.doSpatialDescription(rec, featureid); return
   * false; }
   */
  ...
  public void loadUserFeatures(Rec rec) {
    ...
    // spatialDescriptionManager.addUserFeature(rec, "tuco (tuco@berkeley.edu)",
    // -1);
  }
   ...
   * GeorefManager.GeorefManagerException { if (prefs.locinterp == null ||
   * prefs.locinterp.equalsIgnoreCase("all")) { // I think this won't work:
   * Interpreters will just add more clauses to the // same Rec. // Need to
   * consider a redesign to allow multiple interpreters' georefs. // ClauseSets,
   * but for a new reason. // this.yaleLocInterp.doParsing(this.recset); //
   * this.uiucLocInterp.doParsing(this.recset); //
   * this.TULocInterp.doParsing(this.recset, "Locality", "HigherGeography", //
   * "Country", "State", "County"); } else if
   ...
   * this.yaleLocInterp.doParsing(this.recset); } catch (BGI.BGIException e) {
   * e.printStackTrace(); throw this.new GeorefManagerException(e.toString(),
   * e); } for (Rec rec : this.recset.recs) { if(geoPredoneCheck(rec)){
   * if(populateGeoref(rec)){ return true; } }
   * spatialDescriptionManager.doSpatialDescription(rec); } } else if
