...
public class SpatialDescriptionManager extends BGManager {
  ...
  public static void main(String[] args) throws MalformedURLException,
    ...
    // RecSet rs = new RecSet(
    // "http://bg.berkeley.edu/MaNISGeorefUploadTest1-500.txt", "tab",
  ...
  public void doSpatialDescription(Rec r) {
     ...
     * every combination of FeatureInfos resulting from lookupFeature on the one
     * or two LocSpecs.
     */
    Connection gdb = null;
    ...
    for (Clause clause : r.clauses) { // do feature lookups for all locspecs for
      ...
      if (clause.locType.equalsIgnoreCase("ADM")) {
        ...
        if (clause.locspecs.size() > 0) {
          ProcessStep ps = new ProcessStep(process, version, "");
          clause.locspecs.get(0).featureinfos = gaz.featureQuickLookup(gdb,
              clause.uLocality, "equals-ignore-case", null);
          gaz.addFeatures(userplaces, clause.locspecs.get(0).featureinfos,
        ...
        if (clause.locspecs.size() > 0) {
          ...
          if (clause.uLocality.equalsIgnoreCase(testname)) {
            ...
            clause.locspecs.get(0).featureinfos = gaz.featureQuickLookup(gdb,
                testname, "equals-ignore-case", null);
            gaz.addFeatures(userplaces, clause.locspecs.get(0).featureinfos,
        ...
        if (clause.locspecs.size() > 1) {
          ...
          clause.locspecs.get(1).featureinfos = gaz.featureQuickLookup(gdb,
              clause.locspecs.get(1).featurename, "equals-ignore-case", null);
      ...
      if (gdb != null) { // do other lookup types if the database is defined
        for (LocSpec locspec : clause.locspecs) {
           ...
           * if(locspec.featureinfos==null || locspec.featureinfos.size()==0){ //
           * TODO: other kinds of lookups ps.method=ps.method.concat("Found no
           * features matching "+clause.locspecs.get(0).featurename+" in
           * "+dbname+" using query type contains-all-words."); } else{
           ...
           * TODO: other kinds of lookups ps.method=ps.method.concat("Found no
           * features matching "+clause.locspecs.get(0).featurename+" in
           * "+dbname+" using query type contains-any-words."); } else{
           * ps.method=ps.method.concat("Found "+locspec.featureinfos.size()+"
           * features matching "+locspec.featurename+" in "+dbname+" using query
           ...
           * TODO: other kinds of lookups ps.method=ps.method.concat("Found no
           * features matching "+clause.locspecs.get(0).featurename+" in
           * "+dbname+" using query type contains-phrase."); } else{
           * ps.method=ps.method.concat("Found "+locspec.featureinfos.size()+"
           * features matching "+locspec.featurename+" in "+dbname+" using query
      ...
      if (clause.locspecs.get(0).featureinfos != null) {
        for (int i = 0; i < clause.locspecs.get(0).featureinfos.size(); i++) { // for
                                                                                // every
                                                                                // featureinfo
                                                                                // in
                                                                                // LocSpec1
          ...
          PointRadius pr = null;
          if (clause.locspecs.size() > 1
              && clause.locspecs.get(1).featureinfos.size() > 0) { // there are
                                                                    // features
                                                                    // for the
                                                                    // second
                                                                    // locspec
     ...
     */
    /*
     * Point-radius creation has been attempted for all Clauses in the Rec. Now
     * do a spatial intersection on all combinations of georefs across clauses.
    ...
    int combos = 0;
    int viableclausecount = 0; // number of clauses that have at least one
                                // viable georef.
    int[] gcounts = null; // holds the number of georefs for the clause at that
                          // index
     ...
     * System.out.println("---("+r.georefs.size()+")---"); // TODO: Remove this
     * for(int i=0;i<r.georefs.size();i++){ //
     * System.out.println(r.georefs.get(i).iLocality+":
     * \n"+r.georefs.get(i).pointRadius.toString());
     * System.out.println(r.georefs.get(i).iLocality+":
     * \n"+r.georefs.get(i).toString(true)); } // if(r.georefs.size()==1) //
...
/*
 * for(int i=0;i<locspec.featureinfos.size();i++){ FeatureInfo f =
 * locspec.featureinfos.get(i); if(f.extentInMeters < 1){ // TODO: This same
 * work is being done by the gazetteer. Doesn't // hurt to keep it here for now. //
 * Try making a PointRadius from the encoded Geometry in the event // that the
 * radius given in the database is less than a meter. //
 * log.error("SpatialDescriptionManager found feature extent < 1. Check database
 * for "+f.featureID); GeometryFactory gf = new GeometryFactory(); WKTReader
 * wktreader = new WKTReader(gf); Geometry g=null; try { g =
 * wktreader.read(f.encodedGeometry); Georef georef = new Georef(g,
 * DatumManager.getInstance().getDatum("WGS84")); PointRadius pr =
 * georef.makePointRadius(g, DatumManager.getInstance().getDatum("WGS84"));
 * if(pr.extent>=1) f.extentInMeters=pr.extent; } catch (ParseException e) { //
 * TODO Auto-generated catch block e.printStackTrace(); } if(f.extentInMeters <
 * 1){ // Try getting the best guess uncertainty for the feature based // on its
 * feature type. double d =gaz.lookupBestGuessUncertainty(gdb, f.featureID);
 * if(d>=1) f.extentInMeters=d; } if(f.extentInMeters < 1){ // Try getting the
 * i_feature_footprint.radius of this feature // log.error("Should never have to
 * look up i_feature_footprint.radius. Feature_id: "+f.featureID); // on its
 * feature type. double d =gaz.lookupRadius(gdb, f.featureID); if(d>=1)
 * f.extentInMeters=d; } if(f.extentInMeters < 1){ // The extent is still less
 * than a meter. This feature is a candidate // for correction, supplementation,
 * or removal from the database. log.error("Feature removed for lack of extent:
 * "+f.name+"(FeatureID="+f.featureID+", ExtentInMeters="+f.extentInMeters+")
 * "); locspec.featureinfos.remove(f); i--; } } }
 */
