...
/**
 * <p>The structure to contain spatial descriptions and feature attributes required for 
 * the determination of footprints and uncertainties.</p>
 */
public class FeatureInfo {
	...
	public String classificationTerm; // The Feature Type in which this feature is classified
	public double latitude;           // The decimal latitude of the snapped-to centroid of the feature.
	public double longitude;          // The decimal longitude of the snapped-to centroid of the feature.
	public Datum geodeticDatum;       // The Coordinate Reference System (geodetic datum) of the latitude and longitude.
	public double extentInMeters;     // The distance from the snapped-to centroid of the feature to the further point in the feature, in meters.
	public double coordPrecision;     // Coordinate precision expressed in decimal degrees (e.g., 0.5 means 30 minutes or half a degree).
	public double mapAccuracyInMeters;// Map accuracy in meters (how far off a coordinate can be from the true value based on the original source).
	public String coordSource;        // Required for metadata construction.
	public String encodedGeometry;    // An encoded geometry to hold complex representations of the feature (shapes)
...
//	public ArrayList<MetaData> metadata; // as an ArrayList, metadata could be an event capturing mechanism.
	...
	public static FeatureInfo makeOne(){
		...
		fi.encodedGeometry = new String("POLYGON((588916.4 2432124.4, 588920.7 2432129.9,588928.6 2432123.7,588930.1 2432122.6,588916.4 2432124.4))");
	...
	public FeatureInfo(String name, int fid, String classificationterm, 
			...
			double coordprecision, double mapaccuracyinmeters, String coordsource, String encodedgeometry) {
//		this.name = new String(name);
	...
	public FeatureInfo(FeatureInfo f) {
		...
		if(f.geodeticDatum==null){
			...
			this.geodeticDatum = DatumManager.getInstance().getDatum(f.geodeticDatum.getName());
		}
	...
	public String toString() {
		...
		if( name != null ) {
			s=s.concat(name+ "\n");
		}	else s=s.concat("name not given\n");
	...
	public String toXML(boolean showgeom) {
		...
		if( geodeticDatum!=null && geodeticDatum.getName()!=null && geodeticDatum.getName().length()>0 ){
			s=s.concat("<FEATURE_DATUM>"+geodeticDatum.getName()+"</FEATURE_DATUM>\n");
		} else{
			s=s.concat("<FEATURE_DATUM>not recorded</FEATURE_DATUM>\n");
		}
		s=s.concat("<EXTENTINMETERS>"+extentInMeters+"</EXTENTINMETERS>\n");
		s=s.concat("<COORDINATEPRECISION>"+coordPrecision+"</COORDINATEPRECISION>\n");
		s=s.concat("<MAPACCURACYINMETERS>"+mapAccuracyInMeters+"</MAPACCURACYINMETERS>\n");

		if( coordSource != null && coordSource.length()>0 ) {
			s=s.concat("<COORDINATESOURCE>"+coordSource+"</COORDINATESOURCE>\n");
		} else{
			s=s.concat("<COORDINATESOURCE>not recorded</COORDINATESOURCE>\n");
		}
		
		if( showgeom == true && encodedGeometry != null && encodedGeometry.length()>0 ) {
			s=s.concat("<ENCODEDGEOMETRY>"+encodedGeometry+"</ENCODEDGEOMETRY>\n");
		} else{
