...
public class LocSpec {
    ...
    if (problems == false) { // vlat and vlng both provided
      ...
      if (d != null) {
        if (d.doubleValue() < -90 || d.doubleValue() > 90) { // not in the
          ...
          // log.error("Verbatim latitude ("+locspec.vlat+") is outside the
          // range -90 <=
          // vlat <= 90.");
        }
      }
      try { // Try to make a double out of the vlat value.
        ...
        // log.error("Verbatim longitude ("+locspec.vlng+") isn't a valid
        // decimal
        // longitude.");
      }
      if (d != null) {
        if (d.doubleValue() < -180 || d.doubleValue() > 180) { // not in the
          ...
          // log.error("Verbatim longitude ("+locspec.vlng+") is outside the
          // range -180 <=
          // vlat <= 180.");
        }
      }
  ...
  public void interpretTRS() {
    ...
    if (vsection != null) {
      isection = new String(vsection);
      if (vsubdivision != null) {
        isubdivision = new String(vsubdivision.replaceAll("0.", ".").replace(
            ".25", "1/4").replace(".5", "1/2"));
      }
    }
  }
  ...
  public LocSpecState interpretUTM() {
    ...
    if (iutme != null) {
      if (leftofE.length() == 7) {
        if (intutme.intValue() < 0 || intutme.intValue() > 8340000) { // not in
          // the
          // valid range
          // for a utme
          problems = true;
        ...
        if (intutme.intValue() < 0 || intutme.intValue() > 834000) { // not in
          // the
          // valid range
          // for a utme
          problems = true;
        ...
        if (intutme.intValue() < 0 || intutme.intValue() > 83400) { // not in
          // the
          // valid range
          // for a utme
          problems = true;
    ...
    if (iutmn != null) {
      if (leftofN.length() == 8) {
        if (intutmn.intValue() < 0 || intutmn.intValue() > 10000000) { // not
          // in
          // the valid
          // range for
          // a utmn
        ...
        if (intutmn.intValue() < 0 || intutmn.intValue() > 1000000) { // not in
          // the
          // valid range
          // for a utmn
          problems = true;
        ...
        if (intutmn.intValue() < 0 || intutmn.intValue() > 100000) { // not in
          // the
          // valid range
          // for a utmn
          problems = true;
  ...
  public boolean isHeading() { // Test the validity of the interpreted heading
    // by
