...
/**
 * This class is used for spatial attribute lookup operations against vector and
 * grid layers. The attribute and layer definitions are located in a file named
 * SpatialAttributeLookup.properties in the root src/ directory.
 * 
 * @author aaron
 * 
 */
...
public class SpatialAttributeLookup {
  ...
  private class GridAttribute extends SpatialAttributeLookup.Attribute {
     ...
     * 
     * @param filename
     *          the name of the grid file on disk
     * @param name
     *          the human readable name of this attribute
     */
  ...
  private class VectorAttribute extends SpatialAttributeLookup.Attribute {
     ...
     * 
     * @param table
     *          database table name where this attribute is defined.
     * @param column
     *          database column name where this attribute is defined.
     * @param name
     *          the human readable name of this attribute.
     */
  ...
  public static void main(String[] argv) {
    ...
    for (String attribute : list) {
      ...
      log.info("distance(" + argv[0] + ", " + argv[1] + ", " + attribute + ", "
          + value + ")");
  ...
  /**
   * Constructs the SpatialAttributeLookup object. Starts the log, loads the
   * attribute properties, and connects to the vector database.
   * 
   */
  ...
  /**
   * Calculates the distance between x,y point and a polygon associated with an
   ...
   *          the attribute value
   * 
   * @return kilometers. 0 if x,y in polygon associated with attribute
   *         name/value, -1 if error or if polygon not found, else the distance
   *         in kilometers.
   */
    ...
    try {
      ...
      if (a instanceof VectorAttribute) {
        ...
        query.append("SELECT "
            ...
            + "'POINT(" + lng + " " + lat + ")', " + SRID
            + "), 23033)) / 1000 " + "FROM " + table + " " + "WHERE " + column
            + " = '" + value + "'");
   ...
   * @param lat
   * @param attribute
   *          name of the attribute from which we want the value from
   * @return
   */
  public String lookup(String lng, String lat, String attribute) {
    ...
    try {
      ...
      if (a instanceof VectorAttribute) {
        ...
        query.append("WHERE WITHIN(GEOMFROMTEXT('POINT(" + lng + " " + lat
            + " )'," + SRID + "), the_geom)");
        ...
        point.setLocation(new Double(lng).doubleValue(), new Double(lat)
            .doubleValue());
  ...
  private void loadattributes() {
    ...
    if (vectors != null) {
      for (String layer : vectors) {
        ...
        attributes
            .put(data[VECTOR_ATTRIBUTE_NAME].trim(), this.new VectorAttribute(
                data[VECTOR_TABLE].trim(), data[VECTOR_COLUMN].trim(),
  ...
  private void loadprops() {
    try {
      ...
      InputStream inputStream = loader
          .getResourceAsStream("SpatialAttributeLookup.properties");
