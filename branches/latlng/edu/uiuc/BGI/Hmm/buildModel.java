/**
 * Copyright 2007 Univerimport java.io.BufferedReader;
  ...
  public void parse() throws IOException, FileNotFoundException, Exception {
    ...
    while ((aLine = br.readLine()) != null) {
      ...
      for (int i = 0; i < strArr.length; i++) {
        ...
        System.out.println("strArr.length is:" + strArr.length + "and i is: "
            + i);
        ...
        for (int j = 0; j < childArr.length; j++) {
           ...
           * 
           * for(int r = 0; r < inStr.length(); r++) { if(
           * !Character.isLetterOrDigit(inStr.charAt(r)) &&
           * !Character.isWhitespace(inStr.charAt(r)) ) { resStr = resStr + " " +
           * inStr.charAt(r) + " "; } else { resStr = resStr + inStr.charAt(r); } }
           */
          // end of submit version

          // add space around non-word character except period . and between
          // digit/letter
     ...
     * FileOutputStream("data/model.hmm")); } catch(IOException e) {
     * System.out.println("Can't open model.hmm for writing"); }
     * 
     * bw.write("M= " + M + "\nN= " + N + "\nA:\n"); for(int i = 0; i < N; i++) {
     * for(int j = 0; j < N; j++) { bw.write(A[i][j] + " "); } bw.write("\n"); }
     * bw.write("B:\n"); for(int i = 0; i < N; i++) { for(int j = 0; j < M; j++) {
     * bw.write(B[i][j] + " "); } bw.write("\n"); } bw.write("pi:\n"); for(int i =
     * 0; i < N; i++) { Pi[i] = Pi[i] / nf; bw.write(Pi[i] + " "); }
     * 
     */
    // another way is to use printstream(fileoutputstream). see
    // http://www.javacoffeebreak.com/java103/java103.html. also see java
    // tutorial under sun.
    try {
