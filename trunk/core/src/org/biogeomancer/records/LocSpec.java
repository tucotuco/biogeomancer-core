/***
LocSpec.java
Created 2006-January-15
BioGeomancer Project
TODO: Remove dependencies of methods on interpreted strings
TODO: Complete interpret method.
TODO: Make errors codes powers of two so that they can all be used in a single error state report.
TODO: Integrate process logging so that we can go back and see what has gone wrong during interpretations.
 ***/
...
//In other words, should there be vcoordinateuncertainty, icoordinateuncertainty, etc.
public class LocSpec {
	public LocSpecState state;   // The processing state of the LocSpec
//	public MetaData metadata;    // metadata object to track processing information, such as timestamps and methods.

	public static void main(String[] args) throws Exception {
		if(args.length<2) {
			System.out.println("LocSpec.main requires a language code (en, es, pt, fr) and boolean choice to show geometries as arguments.");
			return;
		}
		...
		if(args[1].equalsIgnoreCase("true")) showgeom=true;
	...
	// The following attributes are to hold the verbatim values parsed from the locality text
	...
	public String vsubdivision;    // a subdivision of the featurename, or of the township, range, section
	public String voffsetew;       // the offset due east or west from the featurename if the Clause.LocType is "orthogonal offsets from a feature"
	public String voffsetewunit;   // the unit of distance of the offsetew
	public String voffsetns;       // the offset due north or south from the featurename if the Clause.LocType is "orthogonal offsets from a feature"
	public String voffsetnsunit;   // the unit of distance of the offsetns
	public String vheadingew;      // the direction, east or west, from the featurename if the Clause.LocType is "orthogonal offsets from a feature"
	public String vheadingns;      // the direction, north or south, from the featurename if the Clause.LocType is "orthogonal offsets from a feature"
	public String velevation;      // the numeric part of the elevation parsed from the Clause (or provided in an elevation attribute)
	public String velevationunits; // an elevation parsed from the Clause (or provided in an elevation attribute)
	public String vlat;            // a latitude parsed from the Clause (or provided in a Latitude attribute)
	public String vlng;            // a longitude parsed from the Clause (or provided in a Longitude attribute)
	public String vdatum;          // a geodetic datum parsed from the Clause (or provided in a Datum attribute)
	public String vutmzone;        // a Universal Tranverse Mercator Zone parsed from the Clause (or provided in a UTM attribute)
	public String vutme;           // a Universal Tranverse Mercator Easting parsed from the Clause (or provided in a UTM attribute)
	public String vutmn;           // a Universal Tranverse Mercator Northing parsed from the Clause (or provided in a UTM attribute)
	public String vtownship;       // a PLSS Township parsed from the Clause
	public String vtownshipdir;    // the direction, north or south, of a PLSS Township parsed from the Clause
	public String vrange;          // a PLSS Range parsed from the Clause
	public String vrangedir;       // the direction, east or west, of a PLSS Range parsed from the Clause
	public String vsection;        // a PLSS Section parsed from the Clause

	// The following attributes are to hold the values interpreted from the verbatim values above
	public String ioffset;         // the linear distance from the featurename expressed as a double
	public String ioffsetunit;     // the unit of distance of the offset
	public String iheading;        // the direction from the featurename
	public String isubdivision;    // a subdivision of the featurename, or of the township, range, section
	public String ioffsetew;       // the offset due east or west from the featurename if the Clause.LocType is "orthogonal offsets from a feature"
	public String ioffsetewunit;   // the unit of distance of the offsetew
	public String ioffsetns;       // the offset due north or south from the featurename if the Clause.LocType is "orthogonal offsets from a feature"
	public String ioffsetnsunit;   // the unit of distance of the offsetns
	public String iheadingew;      // the direction, east or west, from the featurename if the Clause.LocType is "orthogonal offsets from a feature"
	public String iheadingns;      // the direction, north or south, from the featurename if the Clause.LocType is "orthogonal offsets from a feature"
	...
	public String iutmzone;        // a Universal Tranverse Mercator Zone parsed from the Clause
	public String iutme;           // a Universal Tranverse Mercator Easting parsed from the Clause
	public String iutmn;           // a Universal Tranverse Mercator Northing parsed from the Clause
	public String itownship;       // a PLSS Township parsed from the Clause
	public String itownshipdir;    // the direction, north or south, of a PLSS Township parsed from the Clause
	public String irange;          // a PLSS Range parsed from the Clause
	public String irangedir;       // the direction, east or west, of a PLSS Range parsed from the Clause
	public String isection;        // a PLSS Section parsed from the Clause
	public LocSpecManager lsm;     // LocSpecManager to access dictionary, unitconverter, and log errors.

	public ArrayList<FeatureInfo> featureinfos; // a list of featureinfos for which the featurename returned a match from the gazetteer lookup
	...
	public double getOffset(String offsetstring, String offsetunitstring){ 
		if( !this.isOffset(offsetstring, offsetunitstring) ) return 0.0; // Note: 0 isn't necessarily what you want in the case of an invalid offset, you should check before making the call to this method.
		// offsetstring and offsetunitstring contain a valid convertible offset that can be represented in a non-negative double with a standardized unit.
...
//	TODO: TEST ME
	public double getOffsetInMeters(String offsetstring, String offsetunit){
		if( !this.isOffset(offsetstring, offsetunit) ) return 0.0; // Note: 0 isn't necessarily what you want in the case of an invalid offset, you should check before making the call to this method.

		// offsetstring and offsetunit contain a valid convertible offset that can be represented in a non-negative double with a standardized unit.
		if( this.lsm == null) addLocSpecManager(new LocSpecManager());
		UnitConverterManager uc = UnitConverterManager.getInstance();
		Double d = new Double( offsetstring );
		double meters = uc.unit2meters( d.doubleValue(), offsetunit ); // Convert the value of the offset from the original units in ioffsetunit to meters. 
		return meters;
	}
	...
	public double getOffsetEWInMeters(){
		int sign = 1;
		if(this.iheadingew.equalsIgnoreCase("W")) sign=-1;
		return sign*getOffsetInMeters(this.ioffsetew,this.ioffsetewunit);
	}

	public double getOffsetNSInMeters(){
		int sign=1;
		if(this.iheadingns.equalsIgnoreCase("S")) sign=-1;
		return sign*getOffsetInMeters(this.ioffsetns,this.ioffsetnsunit);
	}

	public double getOffsetUncertaintyInMeters(String offsetstring, String offsetunit){
		// Determine the uncertainty associated with the offsetstring and offsetunit. Return that value in meters.
		// Should check that isOffset() == true before calling this method, otherwise the return value of zero will be misleading.
		// Methods derived from Wieczorek, et al. 2004.
		if( !this.isOffset(offsetstring, offsetunit) ) return 0.0;

		// offsetstring and offsetunit contain a valid convertible offset that can be represented in a non-negative double with a standardized unit.
		double offsetuncertainty = 0.0;
		double offset = this.getOffset(offsetstring, offsetunit);
		int sigdigits = 0; // sigdigits is to store the number of significant digits to the right of the decimal.
		double offsetlog10 = Math.log10(offset);
		// First check to see if the verbatim original offset specified significant digits occupied by 0's. These would be 0's following the decimalindicator ('.' or ',').
		if( offsetstring.indexOf(".") != -1 ){ // There is a '.' decimal indicator in the offsetstring.
			sigdigits = offsetstring.trim().length() - offsetstring.trim().indexOf(".")-1; // There are digits to the right of the decimal in the verbatim offset.
		} else if( offsetstring.indexOf(",") != -1 ) { // There is a ',' decimal indicator in the offsetstring.
			sigdigits = offsetstring.trim().length() - offsetstring.trim().indexOf(",")-1; // There are digits to the right of the decimal in the verbatim offset.
		} // Otherwise there is no decimal indicator in the offsetstring.
		if( sigdigits > 0 ) { // If the last digit is a zero, the original was specified to that level of precision.
			if( offsetstring.trim().endsWith("0") ){
				offsetuncertainty = 1.0*Math.pow(10.0,-1.0*sigdigits); // Example: offsetstring = "10.0" offsetuncertainty = 0.1. 
			} else if( offsetlog10 >= 0 && Math.abs(offsetlog10 - Math.rint(offsetlog10)) < 0.0001 ) { // If the ioffset is a positive integer power of ten.
				if( Math.abs(offsetlog10) < 0.0001 ) offsetuncertainty = 1.0; // If that power is zero - offset is 1.
				else offsetuncertainty = Math.pow(10.0, Math.rint(offsetlog10) ); // For 10, 100, 1000, etc. Note that this will be divided by 2 as a last step in the uncertainty calculation in this method.
			} else { // Otherwise get the fractional part of the interpreted offset. We'll use this to determine uncertainty.
				Double d = new Double( offsetstring );
				double intpart = Math.floor( d.doubleValue() ); // The integer part of the offsetstring expressed as a double.
				double fracpart = d.doubleValue()-intpart; // The fractional part of the offsetstring expressed as a double.
				...
				for( int i=0; i<denominators.length; i++ ){
					...
					// If the numerator is within tolerance of being an integer, then the denominator represents the distance precision in the original units.
					if( Math.abs(numerator- nearestint) < 0.001 ) { // This denominator appears to represent a viable fraction.
						offsetuncertainty = 1.0/denominators[i];
						i = denominators.length;
					} 
				}
			}
		} else { // sigdigits = 0, which means the distance is specified as an integer, e.g., 16
			offsetuncertainty = 1.0;
		}
		// Methods taken from Wieczorek, et al. 2004, but modified for fractions to be one-half of that described in the paper, which
	...
	public double getHeadingInDegrees(String heading){
		...
		String headingstring = gd.lookup(heading, SupportedLanguages.english, Concepts.headings, true);
		if( headingstring != null ){ 
			// Heading was found in the GeorefDictionary.
			if( headingstring.equals("N") ) return 0.0;
			if( headingstring.equals("E") ) return 90.0;
			if( headingstring.equals("S") ) return 180.0;
			if( headingstring.equals("W") ) return 270.0;	
			if( headingstring.equals("NE") ) return 45.0;
			if( headingstring.equals("SE") ) return 135.0;
			if( headingstring.equals("SW") ) return 225.0;
			if( headingstring.equals("NW") ) return 315.0;
			if( headingstring.equals("NNE") ) return 22.5;
			if( headingstring.equals("ENE") ) return 67.5;
			if( headingstring.equals("ESE") ) return 112.5;
			if( headingstring.equals("SSE") ) return 157.5;
			if( headingstring.equals("NNW") ) return 337.5;
			if( headingstring.equals("WNW") ) return 292.5;
			if( headingstring.equals("WSW") ) return 247.5;
			if( headingstring.equals("SSW") ) return 202.5;
		}
		// Heading not found in the GeorefDictionary, but it is a valid heading, so it must be a number
		Double dheading = new Double( heading );
		return dheading.doubleValue();
	}
	public double getHeadingInDegrees(){
		if( !isHeading() ) return 0.0; // This is not a valid result, isHeading() should be checked before using a result from this method.
		return getHeadingInDegrees(iheading);
	}
	public double getHeadingUncertaintyInDegrees(String tryheading){
		if( this.lsm == null) addLocSpecManager(new LocSpecManager());
		GeorefDictionaryManager gd = GeorefDictionaryManager.getInstance();
		String headingstring = gd.lookup(tryheading, SupportedLanguages.english, Concepts.headings, true);
		if( headingstring != null ){ // Heading was found in the GeorefDictionary.
			if( headingstring.equals("N") || headingstring.equals("E") || headingstring.equals("S") || headingstring.equals("W") ) return 45.0;	
			if( headingstring.equals("NE") || headingstring.equals("SE") || headingstring.equals("SW") || headingstring.equals("NW") ) return 22.5;
			if( headingstring.equals("NNE") || headingstring.equals("ENE") || headingstring.equals("ESE") || headingstring.equals("SSE") ) return 11.25;
			if( headingstring.equals("NNW") || headingstring.equals("WNW") || headingstring.equals("WSW") || headingstring.equals("SSW") ) return 11.25;
		}
		// Heading not found in the GeorefDictionary, but it is a valid heading, so it must be a number
...
//		double headinglog10 = Math.log10(heading);
		// First check to see if the verbatim original heading specified significant digits occupied by 0's. These would be 0's following the decimalindicator ('.' or ',').
		if( vheading.indexOf(".") != -1 ){ 
			// There is a '.' decimal indicator in the vheading
			sigdigits = vheading.trim().length() - vheading.trim().indexOf("."); // There are digits to the right of the decimal in the verbatim heading
		...
		if( sigdigits > 0 ) { 
			...
			if( vheading.trim().endsWith("0") ) headinguncertainty = 1.0*Math.pow(10.0,-1.0*sigdigits); // Example: vheading = "270.0" headinguncertainty = 0.1
			...
			// Otherwise get the fractional part of the interpreted heading. We'll use this to determine uncertainty.
			double intpart = Math.floor( heading.doubleValue() ); // The integer part of the iheading expressed as a double.
			double fracpart = heading.doubleValue()-intpart; // The fractional part of the iheading expressed as a double.
			// test to see if the fracpart can be turned in to any of the target fractions
			// fracpart/testfraction = integer within a predefined level of tolerance
			double[] denominators = {2.0, 6.0, 60.0, 120.0, 360.0, 3600.0}; // 30', 10', 1', 30", 10", 1"
			for( int i=0; i<denominators.length; i++ ){
				double numerator = fracpart*denominators[i];
				double nearestint = Math.rint( numerator );
				// If the numerator is within tolerance of being an integer, then the denominator represents the distance precision in the original units.
				if( Math.abs(numerator- nearestint) < 0.0000001 ) { // This denominator appears to represent a viable fraction.
	...
	public double getElevationUncertaintyInMeters(){
		// determine the uncertainty associated with the elevation as found in velevation and return that value in meters
		return 0.0; // for now...    
	}
	public boolean isOffset(){ // Test the validity of the interpreted offset as a non-negative distance with distance understood units.
		return this.isOffset( ioffset, ioffsetunit );
	}
	// Test the validity of the offset as a non-negative distance with distance in understood units.
	public boolean isOffset(String offset, String offsetunit){ 
		if( offset == null || offsetunit == null) return false;
		Double d = null;
		...
		if( d.doubleValue() < 0.0 ) return false; // Negative offsets are bad form.
		if( this.lsm == null) addLocSpecManager(new LocSpecManager());
		UnitConverterManager uc = UnitConverterManager.getInstance();
		if( uc.isUnit(offsetunit) == false ){ // offsetunit is not a valid unit found in the UnitConverter.
			if( offsetunit != null ) uc.log("Offsetunit not found: "+offsetunit);
			return false;
		}
		return true; // Looks like a valid offset to me.
	}
	public boolean isHeading(String heading){ // Test the validity of the string as a heading by checking for it in the GeorefDictionary.
		if( heading == null || heading.length()==0 ) return false;
		if( this.lsm == null) addLocSpecManager(new LocSpecManager());
		GeorefDictionaryManager gd = GeorefDictionaryManager.getInstance();
		String headingstring = gd.lookup(heading, SupportedLanguages.english, Concepts.headings, true);
		Double d = null;
		if( headingstring == null ){ // No heading matching headingstring found in the GeorefDictionary.
			...
			try { // Try to make a double out of the iheading value.
				...
				if( d.doubleValue() < 0.0 || d.doubleValue() > 360.0 ) return false; // Heading not between 0 and 360 degrees.
			} catch(Exception e) { // It isn't a valid double value.
				return false;
			}
		}
		return true; // Heading matching headingstring found in the GeorefDictionary or heading is a number.
	}
	public boolean isHeading(){ // Test the validity of the interpreted heading by checking for it in the GeorefDictionary.
		return this.isHeading( this.iheading );
	}

	public boolean areHeadingsOrthogonal(){ // Test the validity of the interpreted offset as a non-negative distance with distance understood units.
		if( !isHeading(iheadingew) || !isHeading(iheadingns) ) return false; // one or the other isn't a valid heading at all
		//		Config config = Config.getInstance();
		//		HashMap locales = (HashMap)config.resources.get("locales");
		if( this.lsm == null) addLocSpecManager(new LocSpecManager());
		GeorefDictionaryManager gd = GeorefDictionaryManager.getInstance();
		String headingstring = gd.lookup(iheadingew, SupportedLanguages.english, Concepts.headings, true);
		// if iheadingew isn't E or W
		if( !headingstring.equalsIgnoreCase("E") && !headingstring.equalsIgnoreCase("W") ) return false;
		headingstring = gd.lookup(iheadingns, SupportedLanguages.english, Concepts.headings, true);
		// if iheadingns isn't N or S
		if( !headingstring.equalsIgnoreCase("N") && !headingstring.equalsIgnoreCase("S") ) return false;
		return true;
	}
	...
	// check validity of ilat as Latitude, ilng as Longitude and build Coordinate with idatum
	//}

	public boolean isUTM(){
		// TODO: determine if the parameters meet the constraints of UTM coordinates.
		return true; // for now...
	}

	/*
	public Coordinate getUTM(){
	// check validity of utmzone, utme, and utmn as UTM coordinates and construct a coordinate with idatum
	}
	 */
	public boolean isTRS(String t, String tdir, String r, String rdir){
		// check that the interpreted PLSS values can make a valid TRS construct (doesn't need to assure that it exists)
		if( t==null || t.length()==0 || tdir==null || tdir.length()==0 || r==null || r.length()==0 || rdir==null || rdir.length()==0) return false;
		Integer tint=null, rint=null;
		try { // Try to make a double out of the iheading value.
			tint = new Integer( t );
			if( tint.intValue() < 0.0 ) return false; // Township numbers are positive.
			rint = new Integer( r );
			if( rint.intValue() < 0.0 ) return false; // Range numbers are positive.
		} catch(Exception e) {
			return false;
		}
		GeorefDictionaryManager gd = GeorefDictionaryManager.getInstance();
		String headingstring = gd.lookup(rdir, SupportedLanguages.english, Concepts.headings, true);
		// if rdir isn't E or W
		if( !headingstring.equalsIgnoreCase("E") && !headingstring.equalsIgnoreCase("W") ) return false;
		headingstring = gd.lookup(tdir, SupportedLanguages.english, Concepts.headings, true);
	...
	public boolean isElevation(String value, String unit){
		...
		if( this.lsm == null) addLocSpecManager(new LocSpecManager());
		UnitConverterManager uc = UnitConverterManager.getInstance();
		if( uc.isUnit(unit) == false ){ // unit is not a valid unit found in the UnitConverter.
			if( unit != null ) uc.log("Unit not found: "+unit);
			return false;
		}
	...
	public void updateInterpretations(){
		if(idatum == null && vdatum != null ) idatum = new String(vdatum);
		if(ielevation == null && velevation != null ) ielevation = new String(velevation);
		if(ielevationunits == null && velevationunits != null ) ielevationunits = new String(velevationunits);
		if(iheading == null && vheading != null ) iheading = new String(vheading);
		if(iheadingew == null && vheadingew != null ) iheadingew = new String(vheadingew);
		if(iheadingns == null && vheadingns != null ) iheadingns = new String(vheadingns);
		if(ilat == null && vlat != null ) ilat = new String(vlat);
		if(ilng == null && vlng != null ) ilng = new String(vlng);
		if(ioffset == null && voffset != null ) ioffset = new String(voffset);
		if(ioffsetunit == null && voffsetunit != null ) ioffsetunit = new String(voffsetunit);
		if(ioffsetew == null && voffsetew != null ) ioffsetew = new String(voffsetew);
		if(ioffsetewunit == null && voffsetewunit != null ) ioffsetewunit = new String(voffsetewunit);
		if(ioffsetns == null && voffsetns != null ) ioffsetns = new String(voffsetns);
		if(ioffsetnsunit == null && voffsetnsunit != null ) ioffsetnsunit = new String(voffsetnsunit);
		if(irange == null && vrange != null ) irange = new String(vrange);
		if(irangedir == null && vrangedir != null ) irangedir = new String(vrangedir);
		if(isection == null && vsection != null ) isection = new String(vsection);
		if(isubdivision == null && vsubdivision != null ) isubdivision = new String(vsubdivision);
		if(itownship == null && vtownship != null ) itownship = new String(vtownship);
		if(itownshipdir == null && vtownshipdir != null ) itownshipdir = new String(vtownshipdir);
		if(iutme == null && vutme != null ) iutme = new String(vutme);
		if(iutmn == null && vutmn != null ) iutmn = new String(vutmn);
		if(iutmzone == null && vutmzone != null ) iutmzone = new String(vutmzone);
	}
	 */
	public void interpretVerbatimAttributes(SupportedLanguages lang){
		...
		interpretUTM();
		/*
		interpretLatLng(lang);
		interpretDatum(lang);
		interpretSubdivision(lang);
		 */
	}
	public void interpretOffset(SupportedLanguages lang) {
		ioffset = ioffsetunit = null;
		if( isOffset(voffset, voffsetunit) == false) return;
		ioffset = new String(voffset);
		String u = UnitConverterManager.getInstance().getStandardUnitString(voffsetunit, lang);
		ioffsetunit = new String(u);
	}	
	public void interpretOffsetEW(SupportedLanguages lang) {
		ioffsetew = ioffsetewunit = null;
		if( isOffset(voffsetew, voffsetewunit) == false) return;
		ioffsetew = new String(voffsetew);
		String u = UnitConverterManager.getInstance().getStandardUnitString(voffsetewunit, lang);
		ioffsetewunit = new String(u);
	}
	public void interpretOffsetNS(SupportedLanguages lang) {
		ioffsetns = ioffsetnsunit = null;
		if( isOffset(voffsetns, voffsetnsunit) == false) return;
		ioffsetns = new String(voffsetns);
		String u = UnitConverterManager.getInstance().getStandardUnitString(voffsetnsunit, lang);
		ioffsetnsunit = new String(u);
	}
	public void interpretHeading(SupportedLanguages lang) {
		iheading = null;
		if( isHeading(vheading) == false ) return;
		String headingstring = GeorefDictionaryManager.getInstance().lookup(vheading, lang, Concepts.headings, true);
	...
	public void interpretHeadingEW(SupportedLanguages lang) {
		iheadingew = null;
		if( isHeading(vheadingew) == false ) return;
		String headingstring = GeorefDictionaryManager.getInstance().lookup(vheadingew, lang, Concepts.headings,true);
		if( headingstring != null ) { // Heading found in GeorefDictionary.
	...
	public void interpretHeadingNS(SupportedLanguages lang) {
		iheadingns = null;
		if( isHeading(vheadingns) == false ) return;
		String headingstring = GeorefDictionaryManager.getInstance().lookup(vheadingns, lang, Concepts.headings, true);
		if( headingstring != null ) { // Heading found in GeorefDictionary.
	...
	public void interpretElevation(SupportedLanguages lang) {
		...
		if( isElevation(velevation, velevationunits) == false) return;
		ielevation = new String(velevation);
		String u = UnitConverterManager.getInstance().getStandardUnitString(velevationunits, lang);
		ielevationunits = new String(u);
	}

	public void interpretTRS() {
		itownship = itownshipdir = irange = irangedir = isection = isubdivision = null;
		if( isTRS(vtownship, vtownshipdir, vrange, vrangedir)==false) return;
		itownship=new String(vtownship);
		String dirstring = GeorefDictionaryManager.getInstance().lookup(vtownshipdir, SupportedLanguages.english, Concepts.headings, false);
		itownshipdir=new String(dirstring);
		irange=new String(vrange);
		dirstring = GeorefDictionaryManager.getInstance().lookup(vrangedir, SupportedLanguages.english, Concepts.headings, false);
		irangedir=new String(dirstring);
		Integer sec = null;
		try { // Try to make a double out of the iheading value.
			sec = new Integer( vsection );
			if( sec.intValue() > 0 && sec.intValue()<37){
				if(vsection!=null) isection=new String(vsection);
			}
		} catch(Exception e) {
		}
		featurename = new String("Township "+itownship+" "+itownshipdir+" Range "+irange+" "+irangedir);
	}
	...
	public LocSpecState interpretUTM() {
		...
		// If the latitudebandletter is an integer, assume the letter is missing and the whole zone is a number
...
//			if(validlatbandletters.indexOf(latitudebandletter) == -1){}
		...
		if( vutme.trim().toUpperCase().charAt(vutme.trim().length()-1) == 'E' ) {
			leftofE = new String(vutme.trim().substring(0,vutme.trim().toUpperCase().indexOf('E')).trim());
		}else {
		...
		if( iutme != null ) {
			if(leftofE.length()==7){
				if( intutme.intValue()<0 || intutme.intValue()>8340000) { // not in the valid range for a utme
					problems = true;
					state = LocSpecState.LOCSPEC_ERROR_UTME_OUT_OF_RANGE;
				}
			} else if(leftofE.length()==6){
				if( intutme.intValue()<0 || intutme.intValue()>834000) { // not in the valid range for a utme
					problems = true;
					state = LocSpecState.LOCSPEC_ERROR_UTME_OUT_OF_RANGE;
				}
			} else if(leftofE.length()==5){
				if( intutme.intValue()<0 || intutme.intValue()>83400) { // not in the valid range for a utme
					problems = true;
					state = LocSpecState.LOCSPEC_ERROR_UTME_OUT_OF_RANGE;
				}
			}
		...
		if( vutmn.trim().toUpperCase().charAt(vutmn.trim().length()-1) == 'N' ) {
			leftofN = new String(vutmn.trim().substring(0,vutmn.trim().toUpperCase().indexOf('N')).trim());
		}else {
		...
		if( iutmn != null ) {
			if(leftofN.length()==8){
				if( intutmn.intValue()<0 || intutmn.intValue()>10000000) { // not in the valid range for a utmn
					problems = true;
					state = LocSpecState.LOCSPEC_ERROR_UTMN_OUT_OF_RANGE;
				}
			} else if(leftofN.length()==7){
				if( intutmn.intValue()<0 || intutmn.intValue()>1000000) { // not in the valid range for a utmn
					problems = true;
					state = LocSpecState.LOCSPEC_ERROR_UTMN_OUT_OF_RANGE;
				}
			} else if(leftofN.length()==6){
				if( intutmn.intValue()<0 || intutmn.intValue()>100000) { // not in the valid range for a utmn
					problems = true;
					state = LocSpecState.LOCSPEC_ERROR_UTMN_OUT_OF_RANGE;
				}				
			}
...
//			log.info("Subdivision interpretation completed successfully.");
	...
	public String toXML(boolean showgeom) {
		...
		if( velevationunits != null && velevationunits.length()>0 ) {
			s=s.concat("<VELEVATIONUNITS>"+velevationunits+"</VELEVATIONUNITS>\n");
			if(ielevationunits!=null && ielevationunits.length()>0) {
				s=s.concat("<IELEVATIONUNITS>"+ielevationunits+"</IELEVATIONUNITS\n");
			}
		}
