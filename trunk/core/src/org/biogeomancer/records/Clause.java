...
public class Clause
// represents information about one atomic interpretable
// unit (Clause) of a Rec.
{
  public ArrayList<Georef> georefs; // generated by shape manager
  // standardized, interpreted version of the locality clause
  public String iLocality;
  // language in which the clause was successfully
  // interpreted. Should be set by the interpreter.
  public SupportedLanguages interpretedInLanguage;
  public ArrayList<LocSpec> locspecs; // generated by locality interp manager
  public String locType; // locality type
  public String sourceField; // input field from which Clause was parsed
  public ClauseState state; // maintains the processing state of the Clause
  // uninterpreted verbatim version of the locality clause
  public String uLocality;
  ...
  public void cleanupClause() {
    for (Georef g : this.georefs) {
      if (g.state != GeorefState.GEOREF_COMPLETED)
        georefs.remove(g);
    }
  }
  ...
  public boolean containsParentFeature(int parentFeatureID) {
    for (LocSpec l : locspecs) {
      if (l.containsParentFeature(parentFeatureID) == true)
        return true;
    }
    return false;
  }
  ...
  public String getSummary(String prefix) {
    String s = new String("\n" + prefix + "Uninterpreted: " + uLocality + "\n"
        ...
        + locType + "\n" + prefix + "Source Field: " + sourceField + "\n"
        + prefix + "Language: " + interpretedInLanguage + "\n" + prefix
        + "Georef count: " + georefs.size() + "\n" + prefix
        + "Clause Georefs:\n");
    for (Georef g : this.georefs) {
  ...
  public String makeInterpretedLocality(FeatureInfo fi1, FeatureInfo fi2,
    ...
    if (locspecs == null || locspecs.size() == 0)
      return null;
    if (locType.equalsIgnoreCase("F") || locType.equalsIgnoreCase("P")
      ...
      if (locspecs.get(0).isubdivision != null
          && locspecs.get(0).isubdivision.length() > 0) {
        iLocality = iLocality.concat(" " + locspecs.get(0).isubdivision);
      }
      ...
      if (locspecs.get(0).ielevation == null
          || locspecs.get(0).ielevationunits == null) {
        iLocality = null;
        return null;
      }
      ...
      iLocality = new String("Lat/Long: " + locspecs.get(0).ilat + " "
          + locspecs.get(0).ilng);
    ...
    if (this.locspecs == null || this.locspecs.isEmpty()) {
      ...
      if (locType == null) {
        ...
        if (locType.toUpperCase().equals("F")
            ...
            || locType.toUpperCase().equals("C")
            || locType.toUpperCase().equals("SP")
            || locType.toUpperCase().equals("2ND")
          ...
          if (this.locspecs.get(0).vsubdivision == null) {
            ...
            s = s.concat("<subdivision>"
                + this.locspecs.get(0).vsubdivision.trim() + "</subdivision>");
          }
        } else if (locType.toUpperCase().equals("TRS")
            || locType.toUpperCase().equals("TRSS")) {
          s = s.concat("<township>" + this.locspecs.get(0).vtownship
              + "</township>");
          s = s.concat("<townshipdir>" + this.locspecs.get(0).vtownshipdir
              + "</townshipdir>");
          s = s.concat("<range>" + this.locspecs.get(0).vrange + "</range>");
          s = s.concat("<rangedir>" + this.locspecs.get(0).vrangedir
              + "</rangedir>");
          s = s.concat("<section>" + this.locspecs.get(0).vsection
              + "</section>");
          if (locType.toUpperCase().equals("TRSS")) {
            if (this.locspecs.get(0).vsubdivision == null) {
              ...
              s = s
                  .concat("<subdivision>"
                      + this.locspecs.get(0).vsubdivision.trim()
                      + "</subdivision>");
            }
          }
        } else if (locType.toUpperCase().equals("UTM")) {
          s = s.concat("<utmzone>" + this.locspecs.get(0).vutmzone
              + "</utmzone>");
          ...
          if (this.uLocality != null)
            s = s.concat(this.uLocality);
        } else if (locType.toUpperCase().equals("E")) {
          s = s.concat("<elevation>" + this.locspecs.get(0).velevation
              + "</elevation>");
          ...
          s = s.concat("<offsetunit>" + this.locspecs.get(0).voffsetunit
              + "</offsetunit>");
          s = s.concat("<heading>" + this.locspecs.get(0).vheading
              + "</heading>");
          ...
          s = s.concat("<feature>" + this.locspecs.get(0).featurename
              + "</feature>");
          s = s.concat("<offsetew>" + this.locspecs.get(0).voffsetew
              + "</offsetew>");
          s = s.concat("<offsetewunit>" + this.locspecs.get(0).voffsetewunit
              + "</offsetewunit>");
          s = s.concat("<headingew>" + this.locspecs.get(0).vheadingew
              + "</headingew>");
          s = s.concat("<offsetns>" + this.locspecs.get(0).voffsetns
              + "</offsetns>");
          s = s.concat("<offsetnsunit>" + this.locspecs.get(0).voffsetnsunit
              + "</offsetnsunit>");
          s = s.concat("<headingns>" + this.locspecs.get(0).vheadingns
              + "</headingns>");
          ...
          if (this.locspecs.size() > 1) {
            s = s.concat("<feature2>" + this.locspecs.get(1).featurename
                + "</feature2>");
          }
          ...
          s = s.concat("<heading>" + this.locspecs.get(0).vheading
              + "</heading>");
          if (this.locspecs.size() > 1) {
            s = s.concat("<path>" + this.locspecs.get(1).featurename
                + "</path>");
          }
  ...
  public String toString() {
    ...
    s = s.concat("Source Field: ");
    if (sourceField != null) {
      s = s.concat(sourceField.toUpperCase() + "\n");
    } else
      s = s.concat("not given\n");
  ...
  public String toXML(boolean showgeom) {
    ...
    if (sourceField != null) {
      s = s.concat("<SOURCEFIELD>" + sourceField.toUpperCase()
          + "</SOURCEFIELD>\n");
    } else {
      s = s.concat("<SOUREFIELD></SOURCEFIELD>\n");
    }
  ...
  public int viableGeorefCount() {
    if (georefs == null || georefs.size() == 0)
      return 0;
    int count = 0;
    for (int i = 0; i < georefs.size(); i++) {
      if (georefs.get(i).state == GeorefState.GEOREF_COMPLETED)
        count++;
    }
    return count;
  }
