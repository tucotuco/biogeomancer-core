...
public class LocSpecManager extends BGManager {
  ...
  public LocSpecState interpretDatum(LocSpec locspec) {
    ...
    if (d == null) {
      ...
      log.error("Verbatim datum (" + locspec.vdatum
          + ") not found in the DatumManager.");
    } else {
  ...
  public LocSpecState interpretElevation(LocSpec locspec) {
    ...
    try { // Try to make a double out of the velevation value.
      ...
      log.error("Verbatim elevation (" + locspec.velevation
    ...
    String u = UnitConverterManager.getInstance().getStandardUnitString(
        locspec.velevationunits, SupportedLanguages.english);
    // Units u =
    // UnitConverterManager.getInstance().getStandardUnit(locspec.velevationunits);
    if (u == null) {
      problems = true;
      locspec.ielevationunits = null;
      locspec.state = LocSpecState.LOCSPEC_ERROR_ELEVATIONUNIT_NOT_FOUND;
      log.error("Verbatim elevation unit (" + locspec.velevationunits
          + ") not found in the GeorefDictionary.");
    } else {
  ...
  public LocSpecState interpretHeading(LocSpec locspec) {
    ...
    String headingstring = GeorefDictionaryManager.getInstance().lookup(
        locspec.vheading, preferredlanguage, Concepts.headings, true);
    if (headingstring != null) { // Heading found in GeorefDictionary.
      ...
      try { // Try to make a double out of the vheading value.
        ...
        // log.error("Verbatim heading ("+locspec.vheading+") not in
        // GeorefDictionary and not a numerrical heading.");
      }
      if (d != null) {
  ...
  public LocSpecState interpretHeadingEW(LocSpec locspec) {
    ...
    String headingewstring = GeorefDictionaryManager.getInstance().lookup(
        locspec.vheadingew, preferredlanguage, Concepts.headings, true);
    if (headingewstring == null) { // Heading found in GeorefDictionary.
      problems = true;
      locspec.iheadingew = null;
      locspec.state = LocSpecState.LOCSPEC_ERROR_HEADING_NOT_FOUND;
      log.error("Verbatim headingew (" + locspec.vheadingew
  ...
  public LocSpecState interpretHeadingNS(LocSpec locspec) {
    ...
    String headingnsstring = GeorefDictionaryManager.getInstance().lookup(
        locspec.vheadingns, preferredlanguage, Concepts.headings, true);
    if (headingnsstring == null) { // Heading found in GeorefDictionary.
      problems = true;
      locspec.iheadingns = null;
      locspec.state = LocSpecState.LOCSPEC_ERROR_HEADING_NOT_FOUND;
      log.error("Verbatim headingns (" + locspec.vheadingns
          + ") not found in GeorefDictionary.");
    } else {
    ...
    if (problems == false) { // vlat and vlng both provided
      ...
      try { // Try to make a double out of the vlat value.
        ...
        log.error("Verbatim latitude (" + locspec.vlat
            + ") isn't a valid decimal latitude.");
      }
      if (d != null) {
        if (d.doubleValue() < -90 || d.doubleValue() > 90) { // not in the
          ...
          log.error("Verbatim latitude (" + locspec.vlat
              + ") is outside the range -90 <= vlat <= 90.");
        }
      }
      try { // Try to make a double out of the vlat value.
        ...
        log.error("Verbatim longitude (" + locspec.vlng
            + ") isn't a valid decimal longitude.");
      }
      if (d != null) {
        if (d.doubleValue() < -180 || d.doubleValue() > 180) { // not in the
          ...
          log.error("Verbatim longitude (" + locspec.vlng
              + ") is outside the range -180 <= vlat <= 180.");
        }
      }
    }
  ...
  public LocSpecState interpretOffset(LocSpec locspec) {
    ...
    String u = UnitConverterManager.getInstance().getStandardUnitString(
        locspec.voffsetunit, SupportedLanguages.english);
    // Units u =
    // UnitConverterManager.getInstance().getStandardUnit(locspec.voffsetunit);
    if (u == null) {
      problems = true;
      locspec.ioffsetunit = null;
      locspec.state = LocSpecState.LOCSPEC_ERROR_OFFSETUNIT_NOT_FOUND;
      log.error("Verbatim offset unit (" + locspec.voffsetunit
          + ") not found in the GeorefDictionary.");
    } else {
  ...
  public LocSpecState interpretOffsetEW(LocSpec locspec) {
    ...
    String u = UnitConverterManager.getInstance().getStandardUnitString(
        locspec.voffsetewunit, SupportedLanguages.english);
    // Units u =
    // UnitConverterManager.getInstance().getStandardUnit(locspec.voffsetewunit);
    if (u == null) {
      problems = true;
      locspec.ioffsetewunit = null;
      locspec.state = LocSpecState.LOCSPEC_ERROR_OFFSETUNIT_NOT_FOUND;
      log.error("Verbatim offsetew unit (" + locspec.voffsetewunit
          + ") not found in the GeorefDictionary.");
    } else {
  ...
  public LocSpecState interpretOffsetNS(LocSpec locspec) {
    ...
    String u = UnitConverterManager.getInstance().getStandardUnitString(
        locspec.voffsetnsunit, SupportedLanguages.english);
    // Units u =
    // UnitConverterManager.getInstance().getStandardUnit(locspec.voffsetnsunit);
    if (u == null) {
      problems = true;
      locspec.ioffsetnsunit = null;
      locspec.state = LocSpecState.LOCSPEC_ERROR_OFFSETUNIT_NOT_FOUND;
      log.error("Verbatim offsetns unit (" + locspec.voffsetnsunit
          + ") not found in the GeorefDictionary.");
    } else {
  ...
  public LocSpecState interpretUTM(LocSpec locspec) {
    ...
    if (locspec == null)
      return null;
    if (locspec.vutmzone == null && locspec.vutme == null
        && locspec.vutmn == null) {
      problems = true;
    ...
    String zonenumber = zone.substring(0, zone.indexOf(latitudebandletter))
        .trim();
    Integer izone = null;
    Integer iutme = null;
    String leftofE = null;
    ...
    if (problems == false) { // vutmzone, vutme, and vutmn all provided
      ...
      // To be unambiguous on its own, the UTM must have a zone with the
      ...
      if (validlatbandletters.indexOf(latitudebandletter) == -1) { // not a
        ...
        log.error("Verbatim utmzone (" + locspec.vutmzone
            + ") doesn't end with a valid latitude band letter.");
      } else {
        ...
        if (izone != null) {
          if (izone.intValue() < 1 || izone.intValue() > 60) { // not in the
            ...
            log.error("Verbatim utmzone (" + locspec.vutmzone
                + ") is outside the range 1 <= vutmzone <= 60.");
          }
        }
      }
      if (locspec.vutme.trim().toUpperCase().charAt(
          locspec.vutme.trim().length() - 1) == 'E') {
        leftofE = new String(locspec.vutme.trim().substring(0,
            locspec.vutme.trim().toUpperCase().indexOf('E')).trim());
      } else {
      ...
      try { // Try to make an integer out of utme.
        ...
        log.error("Verbatim utme (" + locspec.vutme
            + ") isn't an integer, nor an interger followed by 'E'.");
      }
      if (iutme != null) {
        if (iutme.intValue() < 160000 || iutme.intValue() > 834000) { // not in
          ...
          log.error("Verbatim utme (" + locspec.vutme
              + ") is outside the range 160000 <= vutme <= 834000.");
        }
      }
      if (locspec.vutmn.trim().toUpperCase().charAt(
          locspec.vutmn.trim().length() - 1) == 'N') {
        leftofN = new String(locspec.vutmn.trim().substring(0,
            locspec.vutmn.trim().toUpperCase().indexOf('N')).trim());
      } else {
      ...
      if (iutmn != null) {
        if (iutmn.intValue() < 0 || iutmn.intValue() > 10000000) { // not in
          ...
          log.error("Verbatim utmn (" + locspec.vutmn
              + ") is outside the range 0 <= vutmn <= 10000000.");
        }
      }
    }
