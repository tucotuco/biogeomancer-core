...
private String localpath = "c:\\temp";
	...
	public DataHandler georeference(String interpreter,DataHandler indh) {
		...
		if(fa!=null){
		    for(int i=0;i<fa.length;i++){
			    if(fa[i].lastModified()==0L)fa[i].delete();
		    }
		}
		if (indh == null)System.out.println( "no input attachment found");		
	...
	// this main method is a sample service client with service url hard-coded to yale server, 
	// you may want to add the service url as a para.	
	 ...
	 try {
          ...
          call.setTargetEndpointAddress("http://georef22.peabody.yale.edu:8080/axis/services/BGService");	 	  
