...
//	 unfinished rearrange trss by the oder t-r-s-s	   
	...
	public ParsingState parse() {
		...
		{
			String[] ll;
	  	    if(pd.clause.contains(",")&&!pd.clause.matches(".*\\d,\\d.*"))ll=pd.clause.split(",");
	  	    else ll=pd.clause.split("\\s");
	  	    if(ll.length==4){
	  	    	ll[0]+=" "+ll[1];
	  	    	ll[3]=ll[2]+" "+ll[3];
	  	    }
	  	    li.lat=(ll[0].toUpperCase().matches(".+"+parser.regx_HEADING_EW)?ll[ll.length-1]:ll[0]);
	  	    li.lng=(ll[0].toUpperCase().matches(".+"+parser.regx_HEADING_EW)?ll[0]:ll[ll.length-1]).trim();
		}
		else if (localityType=="POM")
		{
		    li.subdivision=pd.words[0]+" "+pd.words[1];
		    li.feature1=pd.clause.replace(pd.words[0],"").replace(pd.words[1],"").trim();
		}
		else if (localityType=="TRS"||localityType=="TRSS")
		{
			...
			if(pd.posH1>=0)li.subdivision=parser.buildString(pd.posH1,pd.words," ");
		}
		...
		else if (localityType=="FH")
		{
			...
			li.feature1+=pd.clause.replace(li.heading,"").replaceAll(parser.regx_OF,"").trim();
		...
		else if (localityType=="FS")
		{
			...
			if(fs.length>1)li.feature1=fs[1];
			else li.feature1=pd.clause.toUpperCase()
			.replaceAll(parser.regx_FS_MASK,"").trim();
			li.subdivision=pd.clause.toLowerCase()
			.replace(li.feature1.toLowerCase(),"").trim();
		...
		else if (localityType=="FO")
		{
			...
			if(li.feature1.length()==1)li.feature1=parser.buildString(pd.posU1+2,pd.words," ");
		...
		else if (localityType=="FOH"||localityType=="JOH")
		{
			...
			li.unit=pd.words[pd.posU1];
			li.heading=pd.words[pd.posH1];
			li.feature1=pd.clause.replace(li.offset, " ")
			.replace(li.unit, " ")
			.replace(li.heading, " ")
			.replaceAll(parser.regx_OF, " ")
			.trim();
			if(localityType=="JOH")
			{
		...
		else if (localityType=="FOO"||localityType=="JOO")
		{
			...
			li.feature1=pd.clause.replace(li.offsetEW, "")
			.replace(li.unit, "")
			.replace(li.headingEW, "")
			.replace(li.offsetNS, "")
			.replace(li.unit, "")
			.replace(li.headingNS, "")
			.replaceAll(parser.regx_OF, " ")
			.replaceAll(parser.regx_AND, " ")
			.trim();
			if(localityType=="JOO")
			{
				...
				.replaceAll(parser.regx_OF, " ")
				.trim()
				.split(parser.regx_J_MASK);
				if(joh.length>2)li.feature1=joh[joh.length-2];
				li.feature2=joh[joh.length-1];
			}
		}
		else if (localityType=="FPOH")
		{
			String[] fpoh=pd.clause.replaceAll("[()]"," ").split(parser.regx_FPOH_MASK);
			li.offset=pd.words[pd.posN1];
			li.unit=pd.words[pd.posU1];
			li.heading=pd.words[pd.posH1];
			li.feature1=fpoh[0]
		...
		{
			String[] j=pd.clause.replaceAll(parser.regx_OF, " ").trim()
			.split(parser.regx_J_MASK);
		...
		else if (localityType=="JH")
		{
			...
			li.feature1=jh[0].replace(li.heading, " ")
		...
		{
			li.feature1=pd.clause.replaceAll(parser.regx_NEAR_MASK, "").replaceAll(parser.regx_OF," ").trim();
		}
		...
		{
			String[]npom=pd.clause.replaceAll(parser.regx_NEAR_MASK, "").split("\\s");			
			li.subdivision=npom[0]+" "+npom[1];
		    li.feature1=pd.clause.replaceAll(parser.regx_NEAR_MASK, "").replace(npom[0],"").replace(npom[1],"").trim();  	
		}
		...
		else if (localityType=="FPH")
		{
			...
			li.heading=pd.words[pd.posH1];
			li.feature1=parser.buildString(0,pd.words," ")
			.replaceAll(parser.regx_OF," ")
			.replace(li.heading," ")
			.replace(li.feature2," ").trim();
		}
