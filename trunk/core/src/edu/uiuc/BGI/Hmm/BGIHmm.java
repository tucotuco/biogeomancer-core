...
public class BGIHmm {
	...
	public void doParsing(RecSet rs) throws BGIHmm.BGIHmmException {
		for (int i = 0; i < rs.recs.size(); i++){
            doParsing((Rec) rs.recs.get(i), "highergeography");
            doParsing((Rec) rs.recs.get(i), "continent");
            doParsing((Rec) rs.recs.get(i), "waterbody");
            doParsing((Rec) rs.recs.get(i), "islandgroup");
            doParsing((Rec) rs.recs.get(i), "island");
            doParsing((Rec) rs.recs.get(i), "country");
            doParsing((Rec) rs.recs.get(i), "stateprovince");
            doParsing((Rec) rs.recs.get(i), "county");
	...
	public void doParsing(Rec r, String sType) throws BGIHmm.BGIHmmException {
		...
		Clause nc;
		if((origStr.toLowerCase().indexOf("no specific") > -1 ) || (origStr.toLowerCase().indexOf("unknown") > -1) ) {
			nc = new Clause();
			nc.locType = "NN";
		...
		try {
			...
			ViterbiCalculator vit = new ViterbiCalculator((Vector) v
					.elementAt(0), hmm);
...
//				System.out.println("stateArray is: " + stateArr[i]);
				...
				if (preParentTag == null) {
					resultString = resultString + "<" + curParentTag + "><"
				...
				for (int j = 0; j < clauseArr.length; j++) {
					...
					if (pTag.equals("ADDR") ) {
						...
						for (int l = 0; l < tmparr.length; l++) {
						    if (Character.isDigit(tmparr[l].charAt(0)) && ls.vtownship == null) {
						            ls.vtownship = tmparr[l];
						    } else if(tmparr[l].toLowerCase().startsWith("n") || tmparr[l].toLowerCase().startsWith("s")) {
						...
						for (int l = 0; l < tmparr.length; l++) {
						    if (Character.isDigit(tmparr[l].charAt(0)) && ls.vrange == null) {
						        ls.vrange = tmparr[l];
						    } else if (tmparr[l].toLowerCase().startsWith("e") || tmparr[l].toLowerCase().startsWith("w")) {
